...
cpdef str get_best_honest_action(list actions, list legal_actions, float hand_strength):
    cdef str action
    cdef int raise_count = actions.count('raise')
    
    if 'check' in legal_actions and hand_strength < 10:
        return 'check'
    
    if 'fold' in legal_actions and hand_strength < 10:
        return 'fold'
    ...
    # If there are fewer than 3 raises, filter out 'all_in'
    if 'raise_100' in legal_actions and raise_count < 3:
        legal_actions = [action for action in legal_actions if action != 'all_in']
    
    cdef int index = round((hand_strength / 20) * (len(legal_actions) - 1))
