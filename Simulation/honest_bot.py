...
from hand_rank_monte_carlo import get_best_honest_action
import random
...
class HonestBot(Player):
    ...
    def get_action(self, game, current_position, effective_stack):
        ...
        if rand <= 2:
            return random.choice(legal_actions) # 0.2% chance to randomly choose action
        hand_strength = evaluate_hand_strength(game,self,20) #get rough evaluation
        
        action = get_best_honest_action(game.actions, legal_actions, hand_strength)
